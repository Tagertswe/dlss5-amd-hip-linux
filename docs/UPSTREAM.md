...
| `0.14` (bundle) | 09-12 | FPS display: update at intervals of at least three seconds, reuse the unchanged text strip, and copy it at the end of the existing output submission instead of a separate synchronous submission. XeSS FG ZeroMV remains enabled in the preset. Remove backup DLLs, logs and shader caches from the bundle; regenerate file checksums. [Magpie-DLSS5-AMD-0.14.zip](https://pan.quark.cn/s/8bbc3033181d), 358,004,639 bytes; SHA256 `14ccde3c752b40821cb9f30024579304627a2499e087e06e9aec399bfe734eed`. No 0.14 tag yet. | Build passed; 671 archive files verified; FPS gain not measured |
| `0.15` (bundle) | 09-13 | Small-window fit: inputs smaller than 1920×1080 (or slightly cropped) are adapted to the fixed DLSS5 geometry before FSR4, preserving aspect ratio (`DLSS5_FIT_INPUT=1` on by default). The preset keeps optical flow (AMDOF) only on the FSR3/DLSS5 stage; FSR4 and XeSS FG use Optical Flow Method None, which looked sharper in the user's test. Also: opt-in C512 FFWD on the FP8 wave-matrix path (`DLSS5_SPLIT_FFWD_FP8`, bit-identical, no time change, default off). Numerically unchanged on the 1080p path. | Onimusha 1080p→2K ≈30 fps; synthetic 4K window test ≈29 network fps |
