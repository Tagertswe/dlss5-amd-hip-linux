...
**[Download v0.2.4 .tar.gz](https://github.com/guentra/dlss5-amd-hip-linux/releases/download/v0.2.4/dlss5-amd-hip-linux.tar.gz)** · [SHA256](https://github.com/guentra/dlss5-amd-hip-linux/releases/download/v0.2.4/dlss5-amd-hip-linux.tar.gz.sha256) · [Release notes](https://github.com/guentra/dlss5-amd-hip-linux/releases/tag/v0.2.4)
...
The offline bench (network only, fixed 1080p input, real converted weights, RX 9070 XT `gfx1201`, warm runs) measures **~50 ms GPU time per inference** on the current build (`0.2.4` enables the byte-exact MH-prod chain; `0.2.3` measured ~63–66 ms, `0.2.1` ~98–105 ms, `0.1.0-poc` 210–216 ms). That is network-only timing, **not in-game FPS**. Kernel work is tracked in the [Changelog](#changelog); data transfers and memory use remain open targets.
