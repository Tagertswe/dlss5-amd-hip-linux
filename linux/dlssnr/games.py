...
def validate_proton(path, *, hip_interop=False):
    """Accept a Wine/Proton runner. hip_interop=True also requires d3dkmt symbols (old HIP trampoline)."""
    root = Path(path).expanduser().resolve()
    advice = 'Choose the Wine/Proton runner folder used by this game (not bin/wine).'
    try:
        ...
        return dict(root=root, wine=base/'bin/wine', ntdll=ntdll, win32u=win32u, dxgi=dxgis[0], hip_interop=hip_interop)
