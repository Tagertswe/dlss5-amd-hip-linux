...
public:
 ...
 void Create(ID3D12CommandQueue*queue,ID3D12Resource*source,
  ...
  if(NativeHipRequested()){
   hip_queue=queue;hip_queue->AddRef();hip.Create();hip_on=true;ready=true;NativeGameFrameStep("hip_backend");return;
  }
 ...
 void RebindSourceAfterCompletion(ID3D12Resource*source){
  ...
  if(hip_on||!resources)return; /* HIP path has no D3D12 encode/decode graph; size is taken from the texture each frame. */
...
public:
 ...
 void ProcessSubmittedFrame(ID3D12Resource*target,D3D12_RESOURCE_STATES source_state,
  ...
  if(hip_on){
   ...
   const auto format=NativePixelFormatFromDxgi(unsigned(desc.Format));
   const size_t bpp=NativePixelBytes(format);
   if(source_state!=target_state)throw std::runtime_error("HIP aliased frame states disagree");
   if(!bpp||!NativePixelGeometry(w,h)||desc.Dimension!=D3D12_RESOURCE_DIMENSION_TEXTURE2D||desc.MipLevels!=1||desc.DepthOrArraySize!=1||desc.SampleDesc.Count!=1)
    throw std::runtime_error("HIP requires a supported typed color texture");
   {static unsigned last_w,last_h;if(w!=last_w||h!=last_h){char t[80];snprintf(t,sizeof t,"hip_frame %ux%u",w,h);NativeGameFrameStep(t);last_w=w;last_h=h;}}
   auto pix=NativeReadSubmittedFrame(hip_queue,target,source_state);
   const size_t pixels=size_t(w)*h;
   if(pix.size()!=pixels*bpp)throw std::runtime_error("HIP readback size");
   std::vector<float>src_rgba,net_rgba(size_t(NativeHipNetWidth)*NativeHipNetHeight*4),net_rgb(size_t(NativeHipNetWidth)*NativeHipNetHeight*3),dst_rgb;
   if(!NativePixelsToRgbaF32(pix.data(),format,w,h,src_rgba))throw std::runtime_error("HIP invalid input pixels");
   NativeResizeChannels(src_rgba.data(),w,h,net_rgba.data(),NativeHipNetWidth,NativeHipNetHeight,4);
   const wchar_t*srgb=_wgetenv(L"DLSS5_CODEC_SRGB");
   if(hip.RunFrame(net_rgba.data(),net_rgb.data(),seed,srgb&&!wcscmp(srgb,L"1")))return;
   dst_rgb.resize(pixels*3);
   NativeResizeChannels(net_rgb.data(),NativeHipNetWidth,NativeHipNetHeight,dst_rgb.data(),w,h,3);
   if(!NativeRgbF32ToPixels(dst_rgb.data(),pix.data(),format,w,h))throw std::runtime_error("HIP invalid output pixels; frame unchanged");
   NativeWriteSubmittedFrame(hip_queue,target,target_state,pix.data(),bpp);
   return;
  }
