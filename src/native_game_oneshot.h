 ...
 static DWORD WINAPI Initialize(void*data){
  ...
  try{
   ...
   {unsigned applied=0;if(FILE*flags=_wfopen(NativeLabPath(L"native-game-flags.txt").c_str(),L"rb")){char line[256];while(fgets(line,sizeof line,flags)){size_t n=strlen(line);while(n&&(line[n-1]=='\n'||line[n-1]=='\r'||line[n-1]==' '))line[--n]=0;if(n<8||strncmp(line,"DLSS5_",6)||!strchr(line,'='))continue;if(!_putenv(line))applied++;}fclose(flags);}
    ...
    char text[160];snprintf(text,sizeof text,"temporal=%u motion=%ux%u render=%ux%u hip=%u",temporal_on?1u:0u,task->temporal.motion_width,task->temporal.motion_height,task->temporal.render_width,task->temporal.render_height,NativeHipRequested()?1u:0u);Log("temporal_config",text);
    ...
    if(!NativeHipRequested()){
     ...
     noise.resize(201326592/4);std::memcpy(noise.data(),noise_bytes.data(),201326592);
    }else Log("hip_path","skip 192MB noise table; ALU prefix");
...
#else
   {std::vector<float>noise;self->frame->Create(self->queue,task->source,noise,NativeLabPath(L"native-color-frame-samegpu").c_str());}
#endif
...
public:
 ...
 /* state: the D3D12 state the upscaler declared for its output (the frame transitions from it and back to it) */
  ...
  try{
   ...
   auto desc=source->GetDesc();
   auto input_check=CheckNativeFrameInput(before,NativePixelFormatFromDxgi(unsigned(desc.Format)),unsigned(desc.Width),desc.Height);
   if(input_check!=NativeFrameInputCheck::valid){
    char text[160];snprintf(text,sizeof text,"%s size=%ux%u bytes=%zu; skip write, keep requesting",
     input_check==NativeFrameInputCheck::black?"black RGB":input_check==NativeFrameInputCheck::nonfinite?"nonfinite":"wrong_size",
     unsigned(desc.Width),unsigned(desc.Height),before.size());
    Log("input_rejected",text);phase=2;return;}
   frame->RebindSourceAfterCompletion(source);
