...
inline std::vector<unsigned char> NativeReadSubmittedFrame(ID3D12CommandQueue*q,ID3D12Resource*source,D3D12_RESOURCE_STATES before){
 ...
 std::vector<unsigned char>result(size_t(w)*h*bpp);void*p=nullptr;D3D12_RANGE range{0,SIZE_T(bytes)};check(readback->Map(0,&range,&p));
 for(UINT y=0;y<h;y++)std::memcpy(result.data()+size_t(y)*w*bpp,static_cast<unsigned char*>(p)+fp.Offset+size_t(y)*fp.Footprint.RowPitch,size_t(w)*bpp);
