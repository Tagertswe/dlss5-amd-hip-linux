...
public:
 ...
 void Create(ID3D12Device*d,const std::vector<ID3D12Resource*>&inputs,const std::wstring&dir){
  ...
  row_pitch=geometry.RowPitch(unorm8_out?4:8);
  desc.Format=DXGI_FORMAT_R16G16B16A16_FLOAT;
  if(unorm8_out){desc={};desc.Dimension=D3D12_RESOURCE_DIMENSION_BUFFER;desc.Width=UINT64(row_pitch)*out_height;desc.Height=1;desc.DepthOrArraySize=desc.MipLevels=1;desc.SampleDesc.Count=1;desc.Layout=D3D12_TEXTURE_LAYOUT_ROW_MAJOR;desc.Flags=D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;}
  ...
  D3D12_UNORDERED_ACCESS_VIEW_DESC uv{};if(unorm_out||unorm8_out){uv.Format=DXGI_FORMAT_R32_TYPELESS;uv.ViewDimension=D3D12_UAV_DIMENSION_BUFFER;uv.Buffer.NumElements=row_pitch*out_height/4;uv.Buffer.Flags=D3D12_BUFFER_UAV_FLAG_RAW;}else{uv.Format=NativeViewFormat(desc.Format);uv.ViewDimension=D3D12_UAV_DIMENSION_TEXTURE2D;}d->CreateUnorderedAccessView(output,nullptr,&uv,cpu);step(d,"uav");
 ...
 D3D12_SUBRESOURCE_FOOTPRINT BufferFootprint()const{return unorm8_out?D3D12_SUBRESOURCE_FOOTPRINT{out_format,out_width,out_height,1,row_pitch}:D3D12_SUBRESOURCE_FOOTPRINT{DXGI_FORMAT_R16G16B16A16_UNORM,out_width,out_height,1,row_pitch};}
};
